...
class AIAgentService {
    ...
    async processRequest(request, onStream) {
        try {
            const emit = (event) => {
                if (onStream)
                    onStream(event);
            ...
            const model = (env_1.env.MODEL || "gpt-4o-mini").includes("gpt-4")
                ? env_1.env.MODEL || "gpt-4o-mini"
                : "gpt-4o-mini";
            ...
            if (message.tool_calls && message.tool_calls.length > 0) {
                ...
                for (let i = 0; i < message.tool_calls.length; i++) {
                    ...
                    const stepNum = i + 1;
                    const toolName = toolCall.type === "function" ? toolCall.function.name : toolCall.function?.name;
                    const toolArgs = toolCall.type === "function" ? JSON.parse(toolCall.function.arguments) : JSON.parse(toolCall.function?.arguments || "{}");
                    // Check if action needs confirmation
                    const needsConfirmation = this.requiresConfirmation(toolName, toolArgs);
                    ...
                    try {
                        const result = await this.executeTool(toolName, toolArgs, request.userId);
                        actions.push({
                            type: toolName,
                            description: stepDescription,
                            result,
                ...
                emit({
                    ...
                    data: {
                        status: "executing",
                        message: "Processing the results and preparing a summary for you...",
                    },
                    timestamp: Date.now(),
                });
    ...
    requiresConfirmation(toolName, _args) {
    ...
    getStepDescription(toolName, args) {
        switch (toolName) {
    ...
    getResultMessage(toolName, result) {
        switch (toolName) {
            case "list_repositories":
            ...
            case "update_github_file":
                return `Successfully updated the file! Commit: ${result.commit?.sha?.substring(0, 7)}`;
            case "create_github_pr":
...
You can execute actions on behalf of the user. Always confirm important actions and provide clear explanations of what you're doing. For urgent GitHub requests, prioritize speed and efficiency.`;
    ...
    async buildUserContext(userId) {
        const repos = await Repo_1.Repo.find({ userId, isActive: true });
        const creditBalance = await credits_service_1.creditsService.getBalance(userId);
        const activeJobs = await Job_1.Job.find({
            repoId: { $in: repos.map((r) => r._id) },
            status: { $in: ["pending", "processing"] },
        });
        const recentEvents = await WebhookEvent_1.WebhookEvent.find({
            repoId: { $in: repos.map((r) => r._id) },
        })
            .sort({ createdAt: -1 })
            .limit(5);
        ...
        return [
            ...
            },
            {
                ...
                function: {
                    ...
                    description: "Update repository settings (auto-update, doc types, branch, notifications)",
                    parameters: {
                        type: "object",
                        properties: {
                            repoId: {
            ...
            },
            {
                ...
                function: {
                    ...
                    description: "Manually trigger documentation generation for a repository",
                    parameters: {
                        type: "object",
                        properties: {
                            repoId: {
                            ...
                            commitSha: {
                                type: "string",
                                description: "Commit SHA to process (optional, uses latest if not provided)",
                            },
                        },
                        required: ["repoId"],
                    },
    ...
    async executeTool(toolName, args, userId) {
        logger_1.logger.info("Executing tool", { toolName, args, userId });
        switch (toolName) {
            case "list_repositories":
                const repos = await Repo_1.Repo.find({ userId, isActive: true });
                return {
            ...
            case "get_repository_details":
                const repo = await Repo_1.Repo.findOne({ _id: args.repoId, userId });
                if (!repo) {
            ...
            case "connect_repository":
                const repoData = await github_service_1.githubService.getRepo(userId, args.owner, args.name);
                const hasWriteAccess = await github_service_1.githubService.checkWriteAccess(userId, args.owner, args.name);
                if (!hasWriteAccess) {
                    throw new Error("Write access required");
                }
                // Note: Full connection requires webhook setup, which is handled in controller
                return {
                    message: "Repository connection initiated. Use the connect endpoint for full setup.",
                    repo: {
                        id: repoData.id,
            ...
            case "disconnect_repository":
                const repoToDisconnect = await Repo_1.Repo.findOne({
                    _id: args.repoId,
                    userId,
                });
                if (!repoToDisconnect) {
                ...
                if (repoToDisconnect.webhookId) {
                    await github_service_1.githubService.deleteWebhook(userId, repoToDisconnect.owner, repoToDisconnect.name, repoToDisconnect.webhookId);
                }
                await Repo_1.Repo.findByIdAndDelete(args.repoId);
                return { message: "Repository disconnected successfully" };
            case "update_repo_settings":
                const repoToUpdate = await Repo_1.Repo.findOne({
                    _id: args.repoId,
                    userId,
                });
                if (!repoToUpdate) {
            ...
            case "get_github_file":
                const fileContent = await github_service_1.githubService.getFileContent(userId, args.owner, args.repo, args.path, args.ref);
            ...
            case "update_github_file":
                ...
                const result = await github_service_1.githubService.updateFile(userId, args.owner, args.repo, args.path, args.message, args.content, sha || "", args.branch || "main");
                return {
            ...
            case "create_github_pr":
                const pr = await github_service_1.githubService.createPullRequest(userId, args.owner, args.repo, args.title, args.body, args.head, args.base || "main");
                return {
            ...
            case "trigger_documentation_generation":
                const repoForGen = await Repo_1.Repo.findOne({ _id: args.repoId, userId });
                if (!repoForGen) {
                    throw new Error("Repository not found");
                }
                ...
                const chosenSha = args.commitSha || repoForGen.lastProcessedCommit || "HEAD";
            ...
            case "get_credit_balance":
                const balance = await credits_service_1.creditsService.getBalance(userId);
                const isBelowThreshold = await credits_service_1.creditsService.isBelowThreshold(userId);
                return {
                    balance,
                    isBelowThreshold,
            ...
            case "add_credits":
                await credits_service_1.creditsService.addCredits(userId, args.amount, "AI Agent top-up");
                const newBalance = await credits_service_1.creditsService.getBalance(userId);
                return {
                    message: "Credits added successfully",
                    balance: newBalance,
                };
            case "get_analytics":
                const analytics = await analytics_service_1.analyticsService.getAnalytics(userId, args.days || 30);
                return {
            ...
            case "get_job_details":
                const job = await Job_1.Job.findOne({ jobId: args.jobId });
                if (!job) {
                    throw new Error("Job not found");
                }
                // Verify user owns the repo
                if (job.repoId) {
                    const jobRepo = await Repo_1.Repo.findById(job.repoId);
