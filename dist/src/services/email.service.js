...
class EmailService {
    ...
    async sendEmail(data) {
        try {
            logger_1.logger.info("Sending email with Resend", {
                to: data.to,
                subject: data.subject,
                hasApiKey: !!env_1.env.RESEND_API_KEY,
            });
    ...
    async sendErrorNotification(to, repoName, error, jobId) {
        const html = `
    ...
    async sendLowCreditsWarning(to, balance) {
        const html = `
    ...
    buildDocUpdateEmail(data) {
        const coverageColor = data.coverageScore >= 0.8
            ? "green"
        ...
        <body>
          <div class="container">
            <div class="wrapper">
              ...
              <div class="content">
                ...
                <div class="section">
                  ...
                  <span class="score-box">${(data.coverageScore * 100).toFixed(0)}%</span>
                </div>
            ...
            .join("")}
                ...
                <div class="section">
                  <h2>Diff Preview</h2>
                  <pre style="white-space:pre-wrap;background:#F3F4F6;padding:12px;border-radius:8px;border:1px solid #E5E7EB;">${data.diffPreview || "(no preview)"}</pre>
                </div>
            ...
            ? `
                  <div class="section">
                    ...
                    ${data.issues
                .map((issue) => `<p style="color: #D97706;">• ${issue}</p>`)
                .join("")}
                  </div>
                `
            : ""}
            ...
            ? `
                <div class="section">
                  ...
                  <p style="margin-top:8px;"><a href="${process.env.APP_URL || "https://app.docr.local"}/app/job/${data.jobId}" style="display:inline-block;padding:10px 14px;border-radius:8px;background:#10B981;color:#fff;text-decoration:none;">View Job Details</a></p>
                </div>
                `
            : ""}
              </div>
